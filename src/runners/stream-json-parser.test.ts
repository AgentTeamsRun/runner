...
test("summarizeToolUse renders human-readable summaries per tool", () => {
  const cwd = "/Users/justin/Project/Me/AgentTeams";
  assert.equal(
    summarizeToolUse("Read", { file_path: `${cwd}/web/src/foo.ts` }, cwd),
    "Read: web/src/foo.ts"
  );
  assert.equal(
    summarizeToolUse("Edit", { file_path: `${cwd}/daemon/src/index.ts` }, cwd),
    "Edit: daemon/src/index.ts"
  );
  assert.equal(
    summarizeToolUse("Bash", { command: "git status\necho done", description: "..." }),
    "Bash: git status"
  );
  assert.equal(
  ...
  assert.equal(
    summarizeToolUse("Grep", { pattern: "foo", path: `${cwd}/src` }, cwd),
    'Grep: "foo" in src'
  );
  ...
  assert.equal(
    summarizeToolUse("TaskUpdate", { taskId: "1", status: "in_progress" }),
...
test("createStreamJsonLineParser handles chunked input and forwards options", () => {
  ...
  const parser = createStreamJsonLineParser(
    (entries) => collected.push(...entries.map((e) => e.message)),
    { cwd: "/repo" }
  );

...
const BACKUP_LOG = fileURLToPath(
  new URL("../../../.agentteams/runner/backup/claude.log", import.meta.url)
);
...
test("backup log replay: default mode hides thinking and renders readable tool lines", (t) => {
  ...
  for (const entry of entries) {
    assert.ok(
      !entry.message.includes("/Users/"),
      `absolute /Users/... path leaked into output: ${entry.message}`
    );
  }
});
...
test("backup log replay: verbose mode re-enables [Thinking] lines", (t) => {
  ...
  assert.ok(
    verboseCount >= defaultCount,
    `verbose entries (${verboseCount}) should be >= default (${defaultCount})`
  );
});

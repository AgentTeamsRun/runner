...
const BACKUP_LOG = fileURLToPath(
  new URL("../../../.agentteams/runner/backup/antigravity.log", import.meta.url)
);
...
test("backup Antigravity log replay keeps only readable whitelist events", async (t) => {
  ...
  for (const pattern of noisePatterns) {
    assert.equal(lines.some((line) => line.includes(pattern)), false, `noise leaked: ${pattern}`);
  }
  ...
  for (const signal of requiredSignals) {
    assert.equal(lines.some((line) => line.startsWith(signal)), true, `missing signal: ${signal}`);
  }
